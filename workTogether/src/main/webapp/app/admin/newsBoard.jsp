  ...
  <main>
    ...
    <section class="userManager">
      <div class="searchcontent">
        ...
        <form action="${pageContext.request.contextPath}/admin/news/newsListOk.ad" method="get">
          <div class="searchbox">
            <select name="column" class="search_column">
              <option value="number" ${param.column == 'number' ? 'selected' : ''}>번호</option>
              <option value="title"  ${param.column == 'title' ? 'selected' : ''}>제목</option>
          ...
          <c:if test="${not empty newsList}">
            <div class="paging_box">
              <ul class="paging">
                <c:if test="${prev}">
                  <li class="prev">
                    <a href="${pageContext.request.contextPath}/admin/news/newsListOk.ad?page=${startPage - 1}">&lt;</a>
                  </li>
                </c:if>

                <c:forEach begin="${startPage}" end="${endPage}" var="p">
                  <li class="page_number ${p == page ? 'active' : ''}">
                    <a href="${pageContext.request.contextPath}/admin/news/newsListOk.ad?page=${p}">${p}</a>
                  </li>
                </c:forEach>

                <c:if test="${next}">
                  <li class="next">
                    <a href="${pageContext.request.contextPath}/admin/news/newsListOk.ad?page=${endPage + 1}">&gt;</a>
                  </li>
                </c:if>
              </ul>
