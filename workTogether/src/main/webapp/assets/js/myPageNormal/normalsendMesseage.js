document.addEventListener('DOMContentLoaded', function() {
	...
	msgContents.forEach((msgContent) => {
		msgContent.addEventListener('click', function() {
			...
			btnReply.addEventListener('click', async () => {
					...
					loadMsgNumber(messageId);
					try {
							const res = await fetch(`/myPageCare/careMsgNumber.mn?msgNumber=${messageId}`, {
								headers: {
	...
	confirmDeleteBtn.addEventListener('click', async function() {
		...
		try {
			const response = await fetch(
				`/myPageCare/careMsgDeleteList.mn?mark_check=${encodeURIComponent(checkedValues.join(","))}`,
				{
					method: "POST",
	...
	async function loadMatch() {
		try {
			const res = await fetch(`/myPageCare/careMsgGetJsList.mn`, {
				headers: {
					Accept: "application/json",
					"X-Requested-With": "XMLHttpRequest",
				},
	...
	async function loadMsgNumber(number) {
		try {
			const res = await fetch(`/myPageCare/careMsgNumber.mn?msgNumber=${number}`, {
				headers: {
					Accept: "application/json",
					"X-Requested-With": "XMLHttpRequest",
				},
