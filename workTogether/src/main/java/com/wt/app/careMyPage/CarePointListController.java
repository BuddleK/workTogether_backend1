...
public class CarePointListController implements Execute{
	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("-=======CarePointListController=======");
		...
		/*
		 * //프로필 사진 담아오기 CareProfileDAO careProfileDAO = new CareProfileDAO();
		...
		// 게시글 목록 조회
		List<CarePointreturnDTO> returnList = carePointreturnDAO.pointReturn(pageMap);
		request.setAttribute("returnList", returnList);
		System.out.println("왤까");
		System.out.println(returnList);
		/*
		 * for(int i = 0 ; i < 4; i++) { System.out.println(markList.get(i)); }
		 */
		...
		int total = carePointreturnDAO.pointReturnCount(usersNumber);
		...
		result.setPath("/app/myPageCare/carePointList.jsp");
