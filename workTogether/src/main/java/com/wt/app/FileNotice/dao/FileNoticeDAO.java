...
public class FileNoticeDAO {
    ...
    public void insert(FileNoticeDTO dto) {
        ...
        try {
            ...
            FileNoticeDTO saved = selectByFilesNumber(dto.getFilesNumber());
    ...
    // 파일 조회 메소드 (PK로 1건)
    public FileNoticeDTO selectByFilesNumber(Integer filesNumber) {
        return sqlSession.selectOne("fileNotice.selectByFilesNumber", filesNumber);
    }

    // 파일 조회 메소드 (뉴스번호로 1건) - 뉴스가 파일 1개만 가질 때
    public FileNoticeDTO selectByNewsNumber(int newsNumber) {
    ...
    public List<FileNoticeDTO> selectRecent(int limit) {
